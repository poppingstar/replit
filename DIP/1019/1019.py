...
plt.figure(figsize=(24,3))  #그림의 크기 설정 너비 24, 높이 3
plt.suptitle('MNIST', fontsize=30)  #제목 설정, MNIST, 폰트 크기 30

for i in range(10):     #10번 반복
    plt.subplot(1,10,i+1)   #1행 10열의 i+1번째 그래프
    plt.imshow(x_train[i], cmap='gray')   #x_train의 i번째 이미지를 흑백으로 표시
    plt.xticks([]); plt.yticks([])  #x축, y축 눈금 표시 안함
    plt.title(str(y_train[i]), fontsize=30) #y_train의 i번째 요소를 제목으로 설정, 폰트 크기 30
plt.show()
(x_train, y_train), (x_test,y_test)=ds.cifar10.load_data()  #CIFAR10 데이터셋을 불러옴
print(x_train.shape, y_train.shape, x_test.shape, y_test.shape)   #불러온 데이터셋의 형태 출력
class_names=['airplane','car','bird','cat','deer','dog','frog','horse','ship','truck']  #클래스 네임 리스트 생성
plt.figure(figsize=(24,3))  #그림의 크기 설정, 너비 24, 높이 3
plt.suptitle('CIFAR-10', fontsize=30)   #제목 설정, CIFAR-10, 폰트 크기 30
for i in range(10):  #10번 반복
    plt.subplot(1,10,i+1)   #1행 10당 열의 이미지 표시,  각 이미지 순차적으로
    plt.imshow(x_train[i])  #x_train의 i번째 이미지 표시
    plt.xticks([]); plt.yticks([])  #x축, y축 눈금 표시 안함
    plt.title(class_names[y_train[i,0]],fontsize=30)    #y_train의 i번째 요소를 제목으로 설정, 폰트 크기 30
plt.show()